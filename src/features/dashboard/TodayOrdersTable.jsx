...
function TodayOrdersTable() {
  ...
  const dataToShow = orders?.slice(
    (currentPage - 1) * TODAY_ORDERS_TO_SHOW,
    currentPage * TODAY_ORDERS_TO_SHOW
  );
  ...
  return (
    <>
      <StyledTable columns="0.3fr 1.4fr 1.4fr 1fr 1fr 0.5fr ">
        <StyledHeading as="h2">Today&apos;s orders</StyledHeading>
        <Table.Header>
          <div>ID</div>
          <div>Customer</div>
          <div>food name</div>
        ...
        <Table.Body
          data={dataToShow}
          render={(order) => (
            <TodayOrderRow key={order.orderid} order={order} />
          )}
        ></Table.Body>
        <Table.Footer>
